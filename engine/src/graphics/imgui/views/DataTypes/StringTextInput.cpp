	...
	namespace graphics
	{
		namespace imgui
		{
			...
			bool SearchBarInput::Render(const char* a_Label, const ImVec2& a_Size, float a_Padding)
			{
				...
				ImVec2 iconPos = ImVec2(cursorPos.x + m_Window.GetWindowPadding().x, cursorPos.y + (inputSize.y - iconSize.y) / 2);
				...
				// Cross icon.
				iconPos = ImVec2((cursorPos.x + inputSize.x) - (m_Window.GetWindowPadding().x + iconSize.x), cursorPos.y + (inputSize.y - iconSize.y) / 2);
