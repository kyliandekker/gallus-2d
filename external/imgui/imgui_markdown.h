...
namespace ImGui
{
	...
	struct MarkdownProjectLinkCallbackData                                 // for both links and images
	{
		const char* text;                               // text between square brackets []
		int                     textLength;
		const char* link;                               // text between brackets ()
	...
	struct MarkdownCheckboxCallbackData                                 // for both links and images
	{
		int size = 0;
		int pos = 0;
		bool enabled = false;
	...
	struct MarkdownFormatInfo
	{
		...
		const char* text;
		size_t textStart, textSize;
	};

	typedef void                MarkdownLinkCallback(MarkdownLinkCallbackData data);
	typedef void                MarkdownProjectLinkCallback(MarkdownProjectLinkCallbackData data);
	typedef void                MarkdownCheckboxCallback(MarkdownCheckboxCallbackData data);
	...
	struct MarkdownConfig
	{
		...
		MarkdownProjectLinkCallback* projectLinkCallback = NULL;
		MarkdownCheckboxCallback* projectCheckboxCallback = NULL;
	...
	struct TextRegion
	{
		...
		bool RenderProjectLinkText(const char* text_, const char* text_end_, const ProjectLink& link_,
			const char* markdown_, const MarkdownConfig& mdConfig_, const char** linkHoverStart_);

		void RenderProjectLinkTextWrapped(const char* text_, const char* text_end_, const ProjectLink& link_,
			const char* markdown_, const MarkdownConfig& mdConfig_, const char** linkHoverStart_, bool bIndentToHere_ = false);
	...
	struct ProjectLink
	{
		enum LinkState {
			NO_LINK,
			HAS_SQUARE_BRACKET_OPEN_SEC,
			HAS_SEPARATOR,
			HAS_SQUARE_BRACKET_CLOSE,
			HAS_SQUARE_BRACKET_CLOSE_SEC,
		};
		LinkState state = NO_LINK;
		TextBlock text;
		TextBlock url;
	...
	inline void StrikeThroughLine(ImColor col_)
	{
		ImVec2 min = ImGui::GetItemRectMin();
		ImVec2 max = ImGui::GetItemRectMax();
	...
	inline void StrikeThroughLineCheckbox(ImColor col_)
	{
		ImVec2 min = ImVec2(25 + ImGui::GetItemRectMin().x, ImGui::GetItemRectMin().y);
		ImVec2 max = ImGui::GetItemRectMax();
	...
	inline void RenderLine(const char* markdown_, Line& line_, TextRegion& textRegion_, const MarkdownConfig& mdConfig_)
	{
		...
		formatInfo.textStart = textStart;
		formatInfo.textSize = textSize;
		formatInfo.text = markdown_;
		...
		else if (line_.isStrikeThrough)         // render emphasis
		{
			formatInfo.type = MarkdownFormatType::STRIKETHROUGH;
			mdConfig_.formatCallback(formatInfo, true);
			const char* text = markdown_ + textStart;
			textRegion_.RenderTextWrapped(text, text + textSize);
		}
		else if (line_.isUnderline)         // render emphasis
		{
			formatInfo.type = MarkdownFormatType::UNDERLINE;
			mdConfig_.formatCallback(formatInfo, true);
			const char* text = markdown_ + textStart;
			textRegion_.RenderTextWrapped(text, text + textSize);
		}
		else if (line_.isVar)         // render var
		{
			formatInfo.type = MarkdownFormatType::VAR;
			mdConfig_.formatCallback(formatInfo, true);
			//const char* text = markdown_ + textStart;
			//textRegion_.RenderTextWrapped(text, text + textSize);
		}
	...
	inline ImVec4 hex_to_rgba(const std::string& hex_color, float alpha = 1.0)
	{
		ImVec4 rgba;

		// Remove '#' if present
		std::string color = hex_color.substr(hex_color.find_first_not_of("#"));
		...
		rgba.x = ((hex_value >> 16) & 0xFF) / 255.0f;
		rgba.y = ((hex_value >> 8) & 0xFF) / 255.0f;
		rgba.z = (hex_value & 0xFF) / 255.0f;
		rgba.w = alpha;
	...
	inline void Markdown(const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_)
	{
		...
		for (int i = 0; i < (int)markdownLength_; ++i)
		{
			...
			switch (textColor.state)
			{
				case TextColor::NONE:
				{
					if (c == '<' && !line.isHeading) // we do not support headings with links for now
					{
						textColor.state = TextColor::LEFT_BRACKET_SEC;
						textColor.color.start = i + 1;
					}
					break;
				}
				case TextColor::LEFT_BRACKET_SEC:
				{
					if (c == '#' && !line.isHeading) // we do not support headings with links for now
					{
						textColor.state = TextColor::SEPARATOR;
					}
					else
					{
						textColor.state = TextColor::SEPARATOR;
				...
				case TextColor::SEPARATOR:
				{
					if (c == '|' && !line.isHeading) // we do not support headings with links for now
					{
						textColor.color.stop = i;
						textColor.state = TextColor::RIGHT_BRACKET;
						textColor.text.start = i;
					}
				}
				case TextColor::RIGHT_BRACKET:
				{
					if (c == '>' && !line.isHeading) // we do not support headings with links for now
					{
						textColor.text.stop = i;

						// render text up to emphasis
						int lineEnd = textColor.color.start - 1;
						...
						line.lastRenderPosition = textColor.text.start;
						line.lineStart = textColor.text.start;
						line.lineEnd = textColor.text.stop;
						ImVec4 color = hex_to_rgba(std::string(markdown_ + textColor.color.start, textColor.color.stop - textColor.color.start));
			...
			switch (checkbox.state)
			{
				case CheckBox::NONE:
				{
					if (var.state == Var::NONE && link.state == Link::NO_LINK && projectLink.state == ProjectLink::NO_LINK && !line.isHeading)
					{
						int next = i + 1;
						int prev = i - 1;
						char c2 = markdown_[next];
						if ((c == '[' && (isspace(c2) || c2 == 'x'))
							&& (int)markdownLength_ > next // emphasis must precede non-whitespace
							&& markdown_[next] != '\n')
						{
							checkbox.state = CheckBox::SPACE_OR_X;
							checkbox.checkbox.start = i;
				...
				case CheckBox::SPACE_OR_X:
				{
					if (isspace(c) || c == 'x')
					{
						checkbox.state = CheckBox::RIGHT;
						if (c == 'x')
						{
				...
				case CheckBox::RIGHT:
				{
					if (c == ']')
					{
						checkbox.checkbox.stop = i;
						checkbox.text.start = i + 1;
						checkbox.state = CheckBox::END;
					}
					break;
				}
				case CheckBox::END:
				{
					if (c == '\n' || i == markdownLength_ - 1)
					{
						checkbox.text.stop = i;
						// render text up to checkbox
						int lineEnd = checkbox.checkbox.start;
						if (lineEnd > line.lineStart)
						{
							line.lineEnd = lineEnd;
							RenderLine(markdown_, line, textRegion, mdConfig_);
						...
						bool initial_checked = checked;
						if (checked)
						{
							ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
						}

						std::string text = std::string(markdown_ + checkbox.text.start, (checkbox.text.stop + 1) - checkbox.text.start);
						if (ImGui::Checkbox(std::string(text + "###" + std::to_string(i) + "_checkbox").c_str(), &checked))
						{
							checkbox.checked = checked;
							if (mdConfig_.projectCheckboxCallback)
							{
								mdConfig_.projectCheckboxCallback({
									(checkbox.checkbox.stop + 1) - checkbox.checkbox.start,
						...
						if (initial_checked)
						{
							ImGui::StrikeThroughLineCheckbox(ImVec4(1, 1, 1, 1));
							ImGui::PopStyleVar();
						}
			...
			if (projectLink.state != ProjectLink::HAS_SQUARE_BRACKET_CLOSE_SEC && checkbox.state != CheckBox::RIGHT)
			{
				...
				switch (link.state)
				{
					case Link::NO_LINK:
					{
						if (c == '[' && markdown_[i + 1] != '[' && !line.isHeading) // we do not support headings with links for now
						{
			...
			{
				case ProjectLink::NO_LINK:
				{
					if (c == '[' && !line.isHeading) // we do not support headings with links for now
					{
						projectLink.state = ProjectLink::HAS_SQUARE_BRACKET_OPEN_SEC;
					}
					break;
				}
				case ProjectLink::HAS_SQUARE_BRACKET_OPEN_SEC:
				{
					if (c == '[' && !line.isHeading) // we do not support headings with links for now
					{
						projectLink.state = ProjectLink::HAS_SEPARATOR;
						projectLink.url.start = i + 1;

						link = Link();
					}
					break;
				}
				case ProjectLink::HAS_SEPARATOR:
				{
					if (c == '|')
					{
						projectLink.state = ProjectLink::HAS_SQUARE_BRACKET_CLOSE;
						projectLink.url.stop = i;
						projectLink.text.start = i + 1;
					}
					else if (c == ']')
					{
						projectLink.state = ProjectLink::HAS_SQUARE_BRACKET_CLOSE_SEC;
						projectLink.url.stop = i;

						projectLink.text.start = projectLink.url.start;
						projectLink.text.stop = projectLink.url.stop;
					}
					break;
				}
				case ProjectLink::HAS_SQUARE_BRACKET_CLOSE:
				{
					if (c == ']')
					{
						projectLink.state = ProjectLink::HAS_SQUARE_BRACKET_CLOSE_SEC;
						projectLink.text.stop = i;
					}
					break;
				}
				case ProjectLink::HAS_SQUARE_BRACKET_CLOSE_SEC:
				{
					if (c == ']')
					{
						// reset emphasis status, we do not support emphasis around links for now
						em = Emphasis();
						// render previous line content
						line.lineEnd = projectLink.url.start - 2;
						RenderLine(markdown_, line, textRegion, mdConfig_);
						line.leadSpaceCount = 0;
						line.isUnorderedListStart = false; // the following text shouldn't have bullets
						ImGui::SameLine(0.0f, 0.0f);
						textRegion.RenderProjectLinkTextWrapped(markdown_ + projectLink.text.start, markdown_ + projectLink.text.start + projectLink.text.size(), projectLink, markdown_, mdConfig_, &linkHoverStart, false);
						ImGui::SameLine(0.0f, 0.0f);
						// reset the link by reinitializing it
						projectLink = ProjectLink();
						line.lastRenderPosition = i;
			...
			switch (em.state)
			{
				...
				case Emphasis::RIGHT:
				{
					if (em.sym == c)
					{
						if (line.emphasisCount < 4 && (i - em.text.stop + 1 == line.emphasisCount))
						{
							// render text up to emphasis
			...
			// Test to see if we have emphasis styling
			switch (strikeThrough.state)
			{
				case StrikeThrough::NONE:
				{
					if (strikeThrough.state == Var::NONE && link.state == Link::NO_LINK && projectLink.state == ProjectLink::NO_LINK && !line.isHeading)
					{
						int next = i + 1;
						int prev = i - 1;
						if ((c == '~' && markdown_[next] == '~')
							...
							&& markdown_[next] != '\t')
						{
							strikeThrough.state = StrikeThrough::LEFT;
							strikeThrough.text.start = i;
							continue;
						}
					}
					break;
				}
				case StrikeThrough::LEFT:
				{
					if (c == '~')
					{
						strikeThrough.state = StrikeThrough::MIDDLE;
						break;
					}
					break;
				}
				case StrikeThrough::MIDDLE:
				{
					strikeThrough.text.start = i;
					strikeThrough.state = StrikeThrough::RIGHT;
				}
				case StrikeThrough::RIGHT:
				{
					if (c == '~')
					{
						strikeThrough.state = StrikeThrough::RIGHT_SEC;
						strikeThrough.text.stop = i;
						break;
					}
				}
				case StrikeThrough::RIGHT_SEC:
				{
					if (c == '~')
					{
						// render text up to strikethrough
						int lineEnd = strikeThrough.text.start - 2;
						if (lineEnd > line.lineStart)
						{
							line.lineEnd = lineEnd;
							RenderLine(markdown_, line, textRegion, mdConfig_);
						...
						line.lastRenderPosition = strikeThrough.text.start - 1;
						line.lineStart = strikeThrough.text.start;
						line.lineEnd = strikeThrough.text.stop;
						RenderLine(markdown_, line, textRegion, mdConfig_);
						ImGui::SameLine(0.0f, 0.0f);
						line.isStrikeThrough = false;
						line.lastRenderPosition = i;
						strikeThrough = StrikeThrough();
					}
					break;
			...
			switch (underline.state)
			{
				case Underline::NONE:
				{
					if (var.state == Var::NONE && link.state == Link::NO_LINK && projectLink.state == ProjectLink::NO_LINK && !line.isHeading)
					{
						int next = i + 1;
						int prev = i - 1;
						if ((c == '~' && markdown_[next] != '~' && markdown_[prev] != '~')
							&& (int)markdownLength_ > next // emphasis must precede non-whitespace
							&& markdown_[next] != ' '
							&& markdown_[next] != '\n'
							&& markdown_[next] != '\t')
						{
							underline.state = Underline::LEFT;
							underline.text.start = i;
							line.emphasisCount = 1;
							continue;
						}
					}
				...
				{
					underline.text.start = i;
					underline.state = Underline::MIDDLE;
					break;
				}
				case Underline::MIDDLE:
				{
					if (c == '~')
					{
						underline.state = Underline::RIGHT;
						underline.text.stop = i;
						// pass through to case Underline::RIGHT
					}
					else
					{
						break;
				...
				case Underline::RIGHT:
				{
					...
					{
						// render text up to emphasis
						int lineEnd = underline.text.start - 1;
						if (lineEnd > line.lineStart)
						{
							line.lineEnd = lineEnd;
							RenderLine(markdown_, line, textRegion, mdConfig_);
						...
						line.lastRenderPosition = underline.text.start - 1;
						line.lineStart = underline.text.start;
						line.lineEnd = underline.text.stop;
						RenderLine(markdown_, line, textRegion, mdConfig_);
						ImGui::SameLine(0.0f, 0.0f);
					...
					else
					{
						...
						// render text up to here
						int start = underline.text.start - line.emphasisCount;
						if (start < line.lineStart)
						{
							line.lineEnd = line.lineStart;
							line.lineStart = start;
			...
			switch (var.state)
			{
				case Var::NONE:
				{
					if (link.state == Link::NO_LINK && !line.isHeading)
					{
						...
						if ((c == '{' && markdown_[next] == '{')
							...
							&& markdown_[next] != '\t')
						{
							var.state = Var::LEFT_BRACKET_SEC;
				...
				case Var::LEFT_BRACKET_SEC:
				{
					if (c == '{')
					{
						var.state = Var::MIDDLE;
						var.text.start = i;
					}
					break;
				}
				...
				case Var::RIGHT_BRACKET_SEC:
				{
					if (c == '}')
					{
						// render text up to emphasis
						int lineEnd = var.text.start - 1;
						if (lineEnd > line.lineStart)
						{
							line.lineEnd = lineEnd;
							RenderLine(markdown_, line, textRegion, mdConfig_);
						...
						line.isVar = true;
						line.lastRenderPosition = var.text.start;
						line.lineStart = var.text.start;
						line.lineEnd = var.text.stop;
						RenderLine(markdown_, line, textRegion, mdConfig_);
						ImGui::SameLine(0.0f, 0.0f);
						line.isVar = false;
						line.lineStart = i;
			...
			if (c == '\n')
			{
				...
				line.lineEnd = i;
				if (em.state == Emphasis::MIDDLE && line.emphasisCount >= 4 &&
		...
		if (em.state == Emphasis::LEFT && line.emphasisCount >= 4)
		{
			ImGui::Separator();
		}
		else
	...
	inline bool TextRegion::RenderProjectLinkText(const char* text_, const char* text_end_, const ProjectLink& link_,
		const char* markdown_, const MarkdownConfig& mdConfig_, const char** linkHoverStart_)
	{
		MarkdownFormatInfo formatInfo;
		formatInfo.config = &mdConfig_;
		formatInfo.type = MarkdownFormatType::PROJECTLINK;
		mdConfig_.formatCallback(formatInfo, true);
		ImGui::PushTextWrapPos(-1.0f);
		ImGui::TextUnformatted(text_, text_end_);
		ImGui::PopTextWrapPos();
		...
		if (bHovered)
		{
			if (ImGui::IsMouseReleased(0) && mdConfig_.projectLinkCallback)
			{
				mdConfig_.projectLinkCallback({ markdown_ + link_.text.start, link_.text.size(), markdown_ + link_.url.start, link_.url.size(), mdConfig_.userData, false });
			}
			if (mdConfig_.tooltipCallback)
			{
				mdConfig_.tooltipCallback({ { markdown_ + link_.text.start, link_.text.size(), markdown_ + link_.url.start, link_.url.size(), mdConfig_.userData, false }, mdConfig_.linkIcon });
	...
	inline void TextRegion::RenderProjectLinkTextWrapped(const char* text_, const char* text_end_, const ProjectLink& link_,
		const char* markdown_, const MarkdownConfig& mdConfig_, const char** linkHoverStart_, bool bIndentToHere_)
	{
		float       scale = ImGui::GetIO().FontGlobalScale;
		float       widthLeft = GetContentRegionAvail().x;
		...
		bool bHovered = RenderProjectLinkText(text_, endLine, link_, markdown_, mdConfig_, linkHoverStart_);
		if (bIndentToHere_)
		{
			float indentNeeded = GetContentRegionAvail().x - widthLeft;
			if (indentNeeded)
		...
		while (endLine < text_end_)
		{
			...
			bool bThisLineHovered = RenderProjectLinkText(text_, endLine, link_, markdown_, mdConfig_, linkHoverStart_);
			bHovered = bHovered || bThisLineHovered;
		}
		if (!bHovered && *linkHoverStart_ == markdown_ + link_.text.start)
		{
	...
	inline void defaultMarkdownFormatCallback(const MarkdownFormatInfo& markdownFormatInfo_, bool start_)
	{
		switch (markdownFormatInfo_.type)
		{
			...
			case MarkdownFormatType::PROJECTLINK:
			{
				if (start_)
				{
					ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_Link]);
				}
				else
				{
					ImGui::PopStyleColor();
					...
					{
						ImGui::UnderLine(ImGui::GetStyle().Colors[ImGuiCol_Link]);
					}
					else
					{
			...
			case MarkdownFormatType::STRIKETHROUGH:
			{
				if (!start_)
				{
					ImGui::StrikeThroughLine(ImVec4(1, 1, 1, 1));
				}
				break;
			}
			case MarkdownFormatType::UNDERLINE:
			{
				if (!start_)
				{
					ImGui::UnderLine(ImVec4(1, 1, 1, 1));
				}
				break;
			}
		}
